...
public class TaskController {
    ...
    @PostMapping
    public ResponseEntity<TaskDefDto> create(@Valid @RequestBody TaskDefDto payload) {
        log.info("Create new task definition request received- payload={}", payload);
        TaskDefDto result = metaDataService.createOrUpdateTaskDefinition(payload);
        URI location = URI.create("/api/tasks/" + result.getName());
        log.info("Create new task definition completed successfully");
